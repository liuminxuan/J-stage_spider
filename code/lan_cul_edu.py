'''
  function:爬取j-stage（言語文化教育研究）杂志论文，并写入Excel文件
  env:python3.6.5
  author:lmx
'''
import time
...
class Jstage:
    def __init__(self):
        #起始地址
        self.start_url = 'https://www.jstage.jst.go.jp/browse/gbkkg/17/' #url前半部分不变，只需修改最后数字
        #请求头，浏览器模拟
        self.headers = {
    ...
    def get_page_url(self):
        ...
        while n<self.page_num:
            yield self.start_url+ str(n) + '/_contents/-char/ja'
            n += 1
            ...
            for ul in soup.find_all('ul', class_='search-resultslisting'):
                for li in ul.find_all('li'):
                    ...
                    try:
                        ...
                        with open("[" + vol + "]" +title + ".pdf", "wb") as pdf:
...
if __name__ == '__main__':
    ...
    top = Jstage()
    try:
        top.saveToExcel('jstage_lan_cul_edu.xlsx')
